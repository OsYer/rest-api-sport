...
export const createNewUser = async (req, res) => {
  ...
  try {
    ...
    // Verificar si el correo electrónico ya está registrado en la tabla Credenciales
    ...
    // Insertar las credenciales del nuevo usuario en la tabla Credenciales
    await pool
      .request()
...
export const login = async (req, res) => {
  ...
  try {
    ...
    const user = result.recordset[0];
    const hashedPassword = user.contraseña; // Se obtiene la contraseña hasheada almacenada en la base de datos
